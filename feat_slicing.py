...
import os
import numpy as np
import src.data_reader.kaldi_io as ko
...
if __name__ == '__main__':
    ...
    data_wd = 'data/'
    curr_wd = os.getcwd()
    orig_train_scp = curr_wd + '/' + data_wd + 'train_spec_cmvn/feats.scp'
    orig_dev_scp = curr_wd + '/' + data_wd + 'dev_spec_cmvn/feats.scp'
    orig_eval_scp = curr_wd + '/' + data_wd + 'eval_spec_cmvn/feats.scp'

    write_wd = 'data_slicing/'
    out_train_scp = curr_wd + '/' + write_wd + \
        'train_spectrogram_cmvn_tensor.scp'
    out_train_ark = curr_wd + '/' + write_wd + \
        'train_spectrogram_cmvn_tensor.ark'
    out_dev_scp = curr_wd + '/' + write_wd + 'dev_spectrogram_cmvn_tensor.scp'
    out_dev_ark = curr_wd + '/' + write_wd + 'dev_spectrogram_cmvn_tensor.ark'
    out_eval_scp = curr_wd + '/' + write_wd + \
        'eval_spectrogram_cmvn_tensor.scp'
    out_eval_ark = curr_wd + '/' + write_wd + \
        'eval_spectrogram_cmvn_tensor.ark'

    train_ark_scp = 'ark:| copy-feats --compress=true ark:- ark,scp:' + \
        out_train_ark + ',' + out_train_scp
    dev_ark_scp = 'ark:| copy-feats --compress=true ark:- ark,scp:' + \
        out_dev_ark + ',' + out_dev_scp
    eval_ark_scp = 'ark:| copy-feats --compress=true ark:- ark,scp:' + \
        out_eval_ark + ',' + out_eval_scp
