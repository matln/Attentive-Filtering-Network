...
sys.path.append(pwd + '/src/data_reader')
sys.path.append(pwd + '/src/attention_neuro')
import argparse
import random
import time
import logging
...
from src.attention_neuro.residual_attention_network import \
     ResidualAttentionModel
from src.attention_neuro.simple_attention_network import AttenResNet, \
     PreAttenResNet, AttenResNet2, AttenResNet4, AttenResNet5
...
# pretrain_path = '/export/b19/jlai/cstr/spoof/model/'
#     'snapshots/attention/attention-2018-07-10_07_21_16-model_best.pth'
...
def main():
    ...
    validation_set = SpoofDataset(
        args.validation_scp, args.validation_utt2label)
    ...
    eval_loader = data.DataLoader(
        eval_set, batch_size=test_batch_size,
        shuffle=False, **params)  # set shuffle to False

    optimizer = optim.Adam(model.parameters(), lr=args.lr, amsgrad=True)
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(
        optimizer, 'min', factor=0.5, patience=1)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        val_loss, eer = validation(
            args, model, device, validation_loader,
            args.validation_scp, args.validation_utt2label, rnn)
    ...
    eval_loss, eval_eer = prediction(
        args, model, device, eval_loader, args.eval_scp,
        args.eval_utt2label, rnn)
    ###########################################################
